...
export class InputManager implements IModule {
    ...
    private commandMapping: Map<string, string> = new Map([
        ['thrust', 'ShiftLeft'],
        ['reverse', 'ControlLeft'],
        ['brake', 'Space'],
        ['pitchUp', 'KeyW'],
    ...
    isCommandPressed(cmd: string): boolean {
        const key = this.commandMapping.get(cmd);
        return key ? this.isKeyPressed(key) : false;
    }
    getCommandKey(cmd: string): string | undefined {
        return this.commandMapping.get(cmd);
    }
    setCommandKey(cmd: string, keyCode: string | null): void {
        if (keyCode === null) {
            this.commandMapping.delete(cmd);
        } else {
            this.commandMapping.set(cmd, keyCode);
        }
    }
    ...
    private onKeyDown(event: KeyboardEvent): void {
        ...
        // Debug: Log key press for verification
        console.log(`[Input] Key Down: ${event.code}`);
    }
